...
function renderSelectProxy(cookieStoreId, proxyId) {
    const proxyOptions = ContainerListModel.proxies.map(p => m('option', {value: p.id, selected: p.id === proxyId}, p.title ? p.title : `${p.host}:${p.port}`));
...
const ContainerListView = {
    oninit: ContainerListModel.loadAll,
    view: () => {
        const items = ContainerListModel.containers.map(renderContainerItem);
        const defaultContainer = renderContainerItem({cookieStoreId:'firefox-default', name: '<default container>' })
